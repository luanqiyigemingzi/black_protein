...
//reg define
reg   [12:0]   start_init_cnt;        //等待初始化计时器
...
//clk时钟频率为250khz，周期为4us 5000*4us = 20ms
//OV5640上电到开始进行IIC配置需等待20ms
...
always @(posedge clk or negedge rst_n) begin
    if(!rst_n)
        ...
        case(init_reg_cnt)
            //先对寄存器进行复位操作，使寄存器恢复初始值
            //寄存器复位操作需要至少1ms，因此分两次配置
            8'd0  : i2c_data <= {16'h300a,8'h0}; //
            8'd1  : i2c_data <= {16'h300b,8'h0}; //
            8'd2  : i2c_data <= {16'h3008,8'h82}; //Bit[7]:复位 Bit[6]:电源控制
            8'd3  : i2c_data <= {16'h3008,8'h02}; //退出休眠模式
            8'd4  : i2c_data <= {16'h3103,8'h02}; //Bit[1]:1 PLL Clock
            //系统输入时钟/FREX/VSYNC/HREF/PCLK/D[9:6]
            8'd5  : i2c_data <= {8'h30,8'h17,8'hff};
            //系统输入时钟/D[5:0]/GPIO1/GPIO0 
            8'd6  : i2c_data <= {16'h3018,8'hff};
            8'd7  : i2c_data <= {16'h3037,8'h13}; //PLL倍频设置
            8'd8  : i2c_data <= {16'h3108,8'h01}; //系统时钟分频
            ...
            8'd32 : i2c_data <= {16'h3a19,8'hf8}; //AEC 增益下限
            ...
            8'd50 : i2c_data <= {16'h4001,8'h02}; //BLC(黑电平校准)起始行
            8'd51 : i2c_data <= {16'h4005,8'h1a}; //BLC(黑电平校准)起始列
            8'd52 : i2c_data <= {16'h3000,8'h00}; //系统块复位控制
            8'd53 : i2c_data <= {16'h3004,8'hff}; //时钟使能控制
            8'd54 : i2c_data <= {16'h4300,8'h6f}; //格式控制 RGB565 //RGB 888 0x23
            ...
            //LENC(镜头校准)设置 16'h5800~16'h583d
            ...
            //SDE(特殊数字效果)控制 16'h5580~16'h558b
            ...
            8'd233: i2c_data <= {16'h3a02,8'h17}; //60Hz最大曝光
            8'd234: i2c_data <= {16'h3a03,8'h10}; //60Hz最大曝光
            8'd235: i2c_data <= {16'h3a14,8'h17}; //50Hz最大曝光
            8'd236: i2c_data <= {16'h3a15,8'h10}; //50Hz最大曝光
            8'd237: i2c_data <= {16'h4004,8'h02}; //BLC(黑电平) 2行
            8'd238: i2c_data <= {16'h4713,8'h03}; //JPEG模式3
            8'd239: i2c_data <= {16'h4407,8'h04}; //量化缩放
            8'd240: i2c_data <= {16'h460c,8'h22};     
            8'd241: i2c_data <= {16'h4837,8'h22}; //DVP CLK分频
            8'd242: i2c_data <= {16'h3824,8'h02}; //DVP CLK分频
            8'd243: i2c_data <= {16'h5001,8'ha3}; //ISP控制
            8'd244: i2c_data <= {16'h3b07,8'h0a}; //帧曝光模式  
            //测试图案使能 
            8'd245: i2c_data <= {16'h503d,8'h00}; //8'h00:正常模式 8'h80:测试图案显示
            //闪光灯控制
            ...
            //只读存储器，防止在case中没有列举的情况出现之前寄存器被重复写入
            default : i2c_data <= {16'h300a,8'h00}; //传感器ID低8位
        endcase
    end
end
